<?php
/**
 * Full EN / AR translations for /agences-de-voyages:
 * positioning body, challenges (titles + bodies + constats), enjeux, proposals.
 */
...
foreach ($byLocale as $locale => $blocks) {
    foreach ($blocks as $dotted => $value) {
        [$section, $key] = explode('.', $dotted, 2);
        ...
        if ($n === 0) {
            ...
            DB::table('content_blocks')->insert([
                'page' => 'agences-de-voyages',
                'section' => $section,
                'key' => $key,
                'locale' => $locale,
                'type' => $fr->type ?? (is_array($value) ? 'json' : 'text'),
                'label' => $fr->label ?? "{$section}.{$key}",
                'value' => $encoded,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            echo "inserted {$dotted} [{$locale}]\n";
        } else {
            echo "updated {$dotted} [{$locale}] rows={$n}\n";
        }
    }
}

// Keep section titles aligned (already present, reaffirm).
$titles = [
    'en' => [
        'hero.title' => 'Travel agencies',
...
foreach ($titles as $locale => $map) {
    foreach ($map as $dotted => $value) {
        [$section, $key] = explode('.', $dotted, 2);
        $n = DB::table('content_blocks')
            ->where('page', 'agences-de-voyages')
            ->where('section', $section)
        ...
        echo "title {$dotted} [{$locale}] rows={$n}\n";
    }
}
