...
const designPath = path.resolve(__dirname, '../public/design-refs/Tourisme la plaisance.png')

const browser = await chromium.launch({ headless: true })
const page = await (
  await browser.newContext({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1 })
).newPage()
await page.goto('http://localhost:3002/tourisme-plaisance', { waitUntil: 'networkidle', timeout: 60000 })
...
const m = await page.evaluate(() => {
  ...
  return {
    ...
    stage: box('.fi2t-gl-plais-stage'),
    impactH2: box('.fi2t-gl-plais-impact__left h2'),
    marina: box('.fi2t-gl-plais-impact__marina'),
    parentH2: box('.fi2t-gl-plais-parent h2'),
    probsH2: box('.fi2t-gl-plais-probs__head h2'),
    prob1: box('.fi2t-gl-plais-probs__card--1'),
    actionsH2: box('.fi2t-gl-plais-actions__left h2'),
    act1: box('.fi2t-gl-plais-actions__item--1'),
    footer: box('.fi2t-footer'),
  }
})
console.log(JSON.stringify(m, null, 2))

const shotPath = path.join(out, 'plaisance-full.png')
...
for name,y0,y1 in [
  ('hero',110,503),('impact',503,1190),('parent',1190,1626),
  ('probs',1626,2584),('actions',2584,3483),('footer',3483,h),
  ('content',503,3483),
]:
